...
#   - The main menu has exactly 13 numbered options, in order 1..13.
#   - The MENU_* constants are sequential 1..13 and dispatched in the case loop.
...
# 1. Exactly 13 numbered options inside prompt_menu
option_count=$(printf '%s\n' "$menu_body" \
    | grep -cE '^[[:space:]]+echo[[:space:]]+"[[:space:]]+[0-9]+\)' || true)
if [[ "$option_count" == "13" ]]; then
    pass "Menu has 13 numbered options"
else
    fail "Menu has $option_count numbered options (expected 13)"
fi

# 2. Numbers are 1..13 in order
nums=$(printf '%s\n' "$menu_body" \
    | grep -oE '"[[:space:]]+[0-9]+\)' \
    | grep -oE '[0-9]+')
expected_nums=$(seq 1 13)
if [[ "$nums" == "$expected_nums" ]]; then
    pass "Options numbered 1..13 in order"
else
    fail "Numbering wrong: got '$(echo "$nums" | tr '\n' ' ')', expected 1..13"
fi

# 3. Final read prompt advertises [1-13]
if grep -qE 'Enter your choice \[1-13\]' "$SETUP_SH"; then
    pass "Final read prompt is [1-13]"
else
    fail "Final read prompt does not say [1-13]"
fi

# 4. MENU_* constants: sequential 1..13, no MENU_BACKEND
constants=$(grep -E '^readonly MENU_[A-Z]+=[0-9]+$' "$SETUP_SH")
const_count=$(printf '%s\n' "$constants" | wc -l | tr -d ' ')
if [[ "$const_count" == "13" ]]; then
    pass "13 MENU_* constants defined"
else
    fail "$const_count MENU_* constants defined (expected 13)"
fi

const_values=$(printf '%s\n' "$constants" | grep -oE '=[0-9]+$' | tr -d '=' | sort -n)
expected_values=$(seq 1 13)
if [[ "$const_values" == "$expected_values" ]]; then
    pass "MENU_* values are 1..13"
else
    fail "MENU_* values not 1..13: got '$(echo "$const_values" | tr '\n' ' ')'"
fi
